...
class BillingView {
    ...
    renderInventory(items, billItems = []) {
        ...
        // Check if we need full re-render or just update classes
        const existingItems = this.inventoryGrid.querySelectorAll('.inventory-item');
        const needsFullRender = existingItems.length !== items.length;
        
        if (needsFullRender) {
            // Full re-render needed (category change, search, initial load)
            ...
            // Wait for layout to stabilize before re-enabling clicks
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    this.inventoryGrid.style.pointerEvents = '';
                    console.log('Inventory grid fully re-rendered');
                });
            });
        } else {
            // Efficient update: only toggle 'selected' class on existing elements
            // This prevents DOM thrashing and layout shifts on mobile
            existingItems.forEach(itemEl => {
                ...
                if (shouldBeSelected && !isCurrentlySelected) {
                    ...
                    console.log(`Added selected to item ${itemId}`);
                } else if (!shouldBeSelected && isCurrentlySelected) {
                    itemEl.classList.remove('selected');
                    console.log(`Removed selected from item ${itemId}`);
                }
            });
            
            console.log('Inventory grid classes updated efficiently');
        }
    }
...
class PDFGenerator {
    ...
    generateHTML() {
        ...
        return `
            <div class="bill-template" style="max-width: 302px; margin: 0 auto; font-family: 'Poppins', sans-serif; padding: 0.2rem; border: none;">
                <div class="bill-header" style="border-bottom: 2px solid #000; padding: 0.3rem 0.15rem 0.4rem 0.15rem; margin-bottom: 0.3rem;">
    ...
    " alt="Sagar Cafe Logo" style="width: 60px; height: 60px; display: block;">
                        ...
                        <div style="flex: 1; display: flex; flex-direction: column; justify-content: center;">
                            <h1 style="font-family: 'Playfair Display', serif; color: #000; margin: 0 0 0.3rem 0; font-size: 1.7rem; line-height: 1.2; font-weight: 700;">
                                Sagar Cafe
                            </h1>
                            <p class="tagline" style="color: #000; font-size: 0.7rem; font-style: italic; margin: 0; line-height: 1.3;">
                                Where Every Wave Brings Flavor
                            </p>
                        </div>
                    </div>
                    <div style="display: flex; justify-content: space-between; padding-top: 0.2rem;">
                        <p style="color: #000; font-size: 0.85rem; margin: 0; line-height: 1.3; padding-bottom: 0.2rem;">☎️ +91 9174849029</p>
                        <p style="color: #000; font-size: 0.85rem; margin: 0; line-height: 1.3; padding-bottom: 0.2rem;">📍 Jabalpur Road, Makronia, Sagar</p>
                    </div>
                </div>

                <div class="bill-info" style="display: block !important; margin-bottom: 0.3rem; font-size: 0.85rem; padding: 0.2rem 0.15rem 0.15rem 0.15rem; color: #000;">
                    ...
                    <div style="display: block; margin-bottom: 0.1rem; clear: both;">
                    ...
                    ${customerContact ? `<div style="display: block; margin-bottom: 0.1rem; clear: both;">
                        <span style="color: #000;"><strong>Contact:</strong> ${customerContact}</span>
                    </div>` : ''}
                </div>

                <div style="padding: 0 0.1rem; overflow-x: visible;">
                <table class="bill-table" style="width: 100%; border-collapse: collapse; margin-bottom: 0.3rem; table-layout: fixed; min-width: 100%;">
                    <thead>
                        <tr style="border-bottom: 1px dotted #000; height: 1.2rem;">
                            <th style="width: 50%; padding: 0.25rem 0.2rem; text-align: left; vertical-align: center; font-weight: 700; font-size: 0.95rem; line-height: 1.3rem; color: #000;">Item</th>
                            <th style="width: 10%; padding: 0.25rem 0.2rem; text-align: center; vertical-align: center; font-weight: 700; font-size: 0.95rem; line-height: 1.3rem; color: #000;">Qty</th>
                            <th style="width: 18%; padding: 0.25rem 0.2rem; text-align: right; vertical-align: center; font-weight: 700; font-size: 0.95rem; line-height: 1.3rem; color: #000;">Price</th>
                            <th style="width: 22%; padding: 0.25rem 0.2rem; text-align: right; vertical-align: center; font-weight: 700; font-size: 0.95rem; line-height: 1.3rem; color: #000;">Amt</th>
                        </tr>
                    </thead>
                    <tbody> 
                        ${items.map(item => `
                            <tr>
                                <td style="padding: 0.2rem 0.2rem; border-bottom: 1px dotted #a19f9fff; color: #000; font-size: 0.9rem; word-wrap: break-word; overflow-wrap: break-word;">
                                    ${item.name}
                                    ${item.isPriceModified ? '<span style="color: #000; font-size: 0.7rem;"> (Mod)</span>' : ''}
                                </td>
                                <td style="padding: 0.2rem 0.2rem; border-bottom: 1px dotted #7a7878ff; text-align: center; color: #000; font-size: 0.9rem;">${item.quantity}</td>
                                <td style="padding: 0.2rem 0.2rem; border-bottom: 1px dotted #7a7878ff; text-align: right; color: #000; font-size: 0.9rem;">₹${Math.round(item.price)}</td>
                                <td style="padding: 0.2rem 0.2rem; border-bottom: 1px dotted #7a7878ff; text-align: right; color: #000; font-size: 0.9rem;">₹${Math.round(item.price * item.quantity)}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>

                <div class="bill-totals" style="margin-left: auto; width: 100%; margin-bottom: 0.3rem; padding: 0 0.15rem;">
                    <div class="bill-totals-total" style="border-top: 2px solid #000; margin-top: 0.2rem; padding: 0.4rem 0.3rem 0.3rem 0.3rem; font-size: 1.3rem; font-weight: 700; color: #000; display: flex; justify-content: space-between;">
                        <span>Total:</span>
                        <span>₹${total.toFixed(2)}</span>
                    </div>
                </div>

                <div class="bill-footer" style="text-align: center; border-top: 2px solid #000; padding: 0.4rem 0.15rem 0.3rem 0.15rem; color: #000; font-size: 0.9rem;">
                    <p class="thank-you" style="font-size: 0.75rem; color: #000; font-weight: 600; margin: 0 0 0.25rem 0; line-height: 1.4;">
                        卐 आयुःसत्त्वबलारोग्यसुखप्रीतिविवर्धनाः। रस्याः स्निग्धाः स्थिरा हृद्या आहाराः सात्त्विकप्रियाः॥ 卐
                    </p>
                    <p style="margin: 0.2rem 0; font-size: 0.85rem; line-height: 1.3; color: #000;">
                        🌐 www.sagarcafe.in
                    </p>
                    <p style="margin: 0.2rem 0; font-size: 0.65rem; line-height: 1.3; color: #000;">
